...
type ResourceMeta = {
  ...
  enKey: boolean, // Use english variant as a key (used in iOS)
  integer: boolean, // Is integer?
};
...
const PLURALS: Plural[] = ['zero', 'one', 'two', 'few', 'many', 'other'];
...
function checkFormatAndGetLocales(row): LocalesMap {
  ...
  if (!localeFound) {
    throw new Error(
      'csv MUST contain at least one locale column. (`en`, `ru`)',
    );
  }
...
function enumerateLocaleValuesInRow(
  locales: LocalesMap,
  row,
  callback: (locale: Locale, value: string) => void,
) {
  ...
  if (isNaN(arrayIndex)) {
    ...
    if (pluralIndex >= 0) {
      ...
      enumerateLocaleValuesInRow(locales, row, (locale, value) => {
        let res: PluralResource = locale.plurals[key];
...
function localeToAndroidFormat(locale: Locale): string {
  ...
  for (let key of keys) {
    ...
    for (let qty of PLURALS) {
      if (value[qty] != null) {
        plural.plurals.push({
          item: [{ _attr: { quantity: qty } }, value[qty]],
        });
      }
    }
...
function localeToAppleStrings(locale: Locale, enLocale: Locale): string {
  ...
  for (let key of keys) {
    ...
    let str =
      JSON.stringify(snakeToCamel(key)) + ' = ' + JSON.stringify(s) + ';';
    if (value.meta.enKey) {
      str =
        JSON.stringify(enLocale.strings[key].value) +
        ' = ' +
        JSON.stringify(s) +
        ';';
    }
  ...
  for (let key of keys) {
    ...
    let str =
      JSON.stringify(snakeToCamel(key)) +
      ' = ' +
...
function localeToAppleStringsDict(locale: Locale, enLocale: Locale): string {
  ...
  for (let key of keys) {
    ...
    let plurals = [
      ...
      { key: 'NSStringFormatValueTypeKey' },
      { string: value.formatter.replace(/%/, '') },
    ];
